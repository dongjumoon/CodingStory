...
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action.equals("free")) {
		...
		} else if (action.equals("wealth")) {
			goWealthBoardPage(request, response);
			...
			} else if (board.equals("wealth")) {
				WealthPostDTO wealthPost = new WealthBoardDAO().getPost(boardId);
				wealthPost.setBoardContent(StringUtil.parseHtml(wealthPost.getBoardContent()));
				String imgFile = wealthPost.getImgFileRealName();
				if (imgFile != null) {
					String[] tokens = imgFile.replace("\\", "/").split("/");
					String fileRealName = tokens[tokens.length - 1];
					wealthPost.setImgFileRealName(fileRealName);
				}
				new WealthBoardDAO().increseViews(boardId);
				post = wealthPost;
			...
			} else if (boardType.equals("wealth")) {
				request.setAttribute("title", "재테크 정보");
			...
			if (boardType.equals("free")) {
				...
				post = new WealthBoardDAO().getPost(boardId);
			...
			} else if (searchArea.equals("wealth")) {
				List<WealthPostDTO> wealthPostList = new WealthBoardDAO().getBoardList(search);
				if (wealthPostList != null) {
					request.setAttribute("wealthPostList", wealthPostList);
				}
				request.setAttribute("title", "재테크 게시판 검색");
				request.getRequestDispatcher("/WEB-INF/views/board/wealth_board.jsp").forward(request, response);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (boardId == null) { //insert
			if (action.equals("free")) { //free insert
				...
				dao = new VideoBoardDAO();
				dto = videoPost;
			} else if (action.equals("wealth")) { // wealth insert
				String saveDirectory = session.getServletContext().getRealPath("images_upload/");
				int maxPostSize = 1024 * 1024 * 10;
				String encoding = "UTF-8";
				...
				try {
					...
					e.printStackTrace();
					session.setAttribute("message", "업로드 실패. 파일 크기는 10MB를 넘을 수 없습니다.");
					response.sendRedirect(request.getContextPath() + "/board/Wealth");
					return;
				}
				...
				WealthPostDTO WealthPost = new WealthPostDTO();
				WealthPost.setBoardTitle(title);
				WealthPost.setBoardContent(content);
				WealthPost.setUserId(user);
				WealthPost.setImgFileName(imgFileName);
				WealthPost.setImgFileRealName(saveDirectory + imgFileRealName);
			...
			if (action.equals("free")) { //free update
			...
			} else if (action.equals("wealth")) { // wealth update
				String saveDirectory = session.getServletContext().getRealPath("images_upload/");
				int maxPostSize = 1024 * 1024 * 10;
				String encoding = "UTF-8";
				...
				WealthPostDTO post = new WealthBoardDAO().getPost(parseBoardId);
	...
	private void goWealthBoardPage(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String page = request.getParameter("page");
		if (page == null) page = "1";
		int pageInt = 1;
		try {
		...
		List<WealthPostDTO> wealthPostList = new WealthBoardDAO().getBoardList(pageInt);
		int pageCount = new WealthBoardDAO().getPageCount(pageInt);
		
		if (wealthPostList != null && pageCount != -1) {
			//페이징에 필요한 정보 얻기 
			int maxPageCount = WealthBoardDAO.MAX_PAGE_COUNT;
