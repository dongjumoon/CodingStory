...
<section class="board">
	<h3><a href="${contextPath}/board/wealth">재테크 정보 '${param.search}' 검색결과</a></h3>
	<table>
		<caption class="screen-out">게시글 목록</caption>
		<thead>
			<tr>
		...
		<tbody>
			<c:if test="${wealthPostList.size() == 0}">
				<tr>
					<td></td>
					<td colspan="4">검색결과가 없습니다.</td>
				</tr>
			</c:if>
			<c:forEach var="post" items="${wealthPostList}">
			<tr>
				<td>${post.boardId}</td>
				<td><a href="wealth/view?boardId=${post.boardId}" title="내용보기">${post.boardTitle}</a></td>
				<td><span>${post.userId}</span></td>
				<td>${post.boardDate}</td>
				<td>${post.boardViews}</td>
			</tr>
	...
	<div class="board-btn-box">
		<a href="${contextPath}/board/wealth/write.user">글쓰기</a>
	</div>
</section>